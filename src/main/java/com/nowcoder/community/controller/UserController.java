...
import com.nowcoder.community.services.FollowService;
...
public class UserController implements CommunityConstant {
    ...
    public String changePassword(String oldPassword, String newPassword,String truePassword, Model model){
        User user = hostHolder.getUser();
...
//        }
        if(newPassword.length()<8){
            model.addAttribute("lengthError", "新密码长度不能小于8位!!");
            return "/site/setting";
        }
        if(!newPassword.equals(truePassword)){
            model.addAttribute("changeError", "两次输入的密码不一致!");
            return "/site/setting";
        }
